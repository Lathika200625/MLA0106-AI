...
README.md
3 hours ago
Screenshot 2026-02-13 143520.png
5 hours ago
Screenshot 2026-02-13 144035.png
5 hours ago
Repository files navigation
README
Breath First Search(BFS):- BFS(Graph, S): create empty queue Q mark S as visited enqueue S into Q while Q is not empty: V = dequeue Q print V

for each neighbor U of V:
    if U not visited:
        mark U as visited
        enqueue U into Q
DEPTH FIRST SEARCH(DFS):- DFS(Graph): for each vertex V in Graph: mark V as not visited

for each vertex V in Graph: if V is not visited: DFS_Visit(V)

DFS_Visit(V): mark V as visited print V

for each neighbor U of V: if U is not visited: DFS_Visit(U)

UNIFORM COST SEARCH(UCS):- UCS(Graph, Start, Goal):

create priority queue PQ create cost table

cost(Start) = 0 insert Start into PQ
...
create OPEN priority queue (based on heuristic h(n)) create CLOSED set
...
create empty queue Q create VISITED set

initial_state = (0, 0) enqueue initial_state into Q mark initial_state as visited
...
if node is terminal OR depth == 0: return value(node)
...
if terminal(node) OR depth == 0: return heuristic(node)

if isMax: value = -∞ for child in children(node): value = max(value, ALPHA_BETA(child, depth-1, alpha, beta, false)) alpha = max(alpha, value) if alpha >= beta: break // beta cut-off return value

else: value = +∞ for child in children(node): value = min(value, ALPHA_BETA(child, depth-1, alpha, beta, true)) beta = min(beta, value) if beta <= alpha: break // alpha cut-off return value
...
If the color is valid → Move to next region.

Step 6:

If no color is valid → Backtrack (change previous region’s color).
...
If all regions are colored successfully → Print solution.

Step 9: Stop.

prolog 2: Step 1: Start Step 2:
...
prolog 6: Define facts using orbits(Object, Body)
...
Check if a character belongs to {a, e, i, o, u, A, E, I, O, U}
...
Display total vowel count. prolog 8: Define gender facts
